...
void rockchip_drm_fb_destroy_work(struct work_struct *work)
{
	struct rockchip_drm_fb *fb;

	fb = container_of(to_delayed_work(work), struct rockchip_drm_fb, destroy_work);
...
static void rockchip_drm_fb_destroy(struct drm_framebuffer *fb)
{
	...
	if (rockchip_fb_is_logo(fb))
		schedule_delayed_work(&rockchip_fb->destroy_work, HZ);
	else
		  ...
		  unsigned int num_planes)
{
	...
	if (obj) {
		...
		logo->count++;
		INIT_DELAYED_WORK(&rockchip_fb->destroy_work, rockchip_drm_fb_destroy_work);
#endif
	} else {
		ret = -EINVAL;
