			     ...
			     const struct vop2_zpos *vop2_zpos)
{
	...
	struct drm_plane *plane;
	struct vop2_plane_state *vpstate;
	int pixel_alpha_en = false;
	int premulti_en = true;
	int mixer_id;
	uint32_t offset;
	int i;
	bool bottom_layer_alpha_en = false;
	bool bottom_layer_no_premulti = false;
	...
	drm_atomic_crtc_for_each_plane(plane, &vp->crtc) {
		...
		if (vpstate->zpos == 0) {
			...
			/*
			 * If bottom layer have alpha effect [except cluster layer,
			 * because cluster have deal with bottom layer alpha value
			 * at cluster mix], bottom layer mix need deal with alpha.
			 */
			if (!vop2_cluster_window(win)) {
				if (vpstate->global_alpha != 0xff || pixel_alpha_en)
					bottom_layer_alpha_en = true;
				dst_global_alpha = vpstate->global_alpha;
				if (plane->state->pixel_blend_mode == DRM_MODE_BLEND_COVERAGE)
					bottom_layer_no_premulti = true;
			}
			break;
		}
	}
	...
	/* transfer alpha value to next mixer when is no premulti alpha */
	if (mixer_id != 0) {/* except layer mix0 */
		offset = (mixer_id - 1) * 0x10;
		alpha.src_alpha_ctrl.val = 0;
		alpha.dst_alpha_ctrl.val = 0;
		if (pixel_alpha_en && bottom_layer_no_premulti) {
	...
	for (i = 1; i < vp->nr_layers; i++) {
		...
		alpha_config.src_premulti_en = premulti_en;
		if (i == 1) {/* Cd = Cs + (1 - As) * Cd * Agd */
			alpha_config.dst_premulti_en = false;
			if (mixer_id == 0 || mixer_id == 3) {
				alpha_config.src_pixel_alpha_en = pixel_alpha_en;
				alpha_config.dst_pixel_alpha_en = bottom_layer_alpha_en;
			} else {
				alpha_config.src_pixel_alpha_en = pixel_alpha_en;
	...
	if (vp->id == 0) {
		/* for HDR10 Mix, the alpha cal only can from src layer */
		alpha.src_alpha_ctrl.bits.blend_mode = ALPHA_PER_PIX;
	...
	if (vp->nr_layers == 1) {
		...
		if (plane->state->pixel_blend_mode == DRM_MODE_BLEND_PREMULTI)
			premulti_en = 1;
		else if (plane->state->pixel_blend_mode == DRM_MODE_BLEND_COVERAGE)
			premulti_en = 0;
		else
			pixel_alpha_en = 0;
	}

	alpha.bg_alpha_ctrl.val = 0;
	alpha.bg_alpha_ctrl.bits.alpha_en = pixel_alpha_en;
	alpha.bg_alpha_ctrl.bits.alpha_mode = ALPHA_PER_PIX;
	alpha.bg_alpha_ctrl.bits.alpha_pre_mul = !premulti_en;
	alpha.bg_alpha_ctrl.bits.alpha_sat_mode = ALPHA_SATURATION;
	alpha.bg_alpha_ctrl.bits.glb_alpha = 0xff;
	VOP_MODULE_SET(vop2, vp, bg_mix_ctrl, alpha.bg_alpha_ctrl.val);
}
