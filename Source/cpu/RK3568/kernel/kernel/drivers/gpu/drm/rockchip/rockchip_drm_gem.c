...
static void
rockchip_gem_object_exported_dma_buf_free(struct drm_gem_object *obj)
{
	if (obj->dma_buf) {
...
static void
rockchip_gem_object_handle_free(struct drm_gem_object *obj)
{
...
static void
rockchip_gem_object_handle_put_unlocked(struct drm_gem_object *obj)
{
	struct drm_device *dev = obj->dev;
	bool final = false;
	...
	if (--obj->handle_count == 0) {
		rockchip_gem_object_handle_free(obj);
		rockchip_gem_object_exported_dma_buf_free(obj);
		final = true;
	}
	mutex_unlock(&dev->object_name_lock);

	if (final)
		rockchip_gem_object_handle_put_unlocked(obj);
}

static void
rockchip_gem_remove_buf_handle_locked(struct drm_prime_file_private *prime_fpriv,
...
static void
rockchip_gem_remove_prime_handle(struct drm_gem_object *obj,
				 struct drm_file *filp, int handle)
{
	/*
	...
	if (obj->dma_buf) {
		rockchip_gem_remove_buf_handle_locked(&filp->prime,
						      obj->dma_buf, handle);
	}
	mutex_unlock(&filp->prime.lock);
}

static int rockchip_gem_object_release_handle(int id, void *ptr, void *data)
{
	...
	if (drm_core_check_feature(dev, DRIVER_PRIME))
		rockchip_gem_remove_prime_handle(obj, file_priv, id);
...
static int rockchip_gem_handle_delete(struct drm_file *filp, u32 handle)
{
	struct drm_gem_object *obj;
	...
	rockchip_gem_object_release_handle(handle, obj, filp);
...
int rockchip_gem_close_ioctl(struct drm_device *dev, void *data,
			     struct drm_file *file_priv)
{
	...
	ret = rockchip_gem_handle_delete(file_priv, args->handle);
