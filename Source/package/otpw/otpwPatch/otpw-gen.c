...
/* Matrix changes start */
int AesDebugPrint(AesDebugLvl_e debugLvl, const char *fmt, ...)
{
    UNUSED_ARG(debugLvl);
    UNUSED_ARG(fmt);
    return (0);
...
int main(int argc, char **argv)
{
  ...
  int help = 0;
  /* Matrix changes start */
  char prefix_str[PREFIX_SIZE+1], prefix_str1[PREFIX_SIZE];
  unsigned char prefix_index;
  int rows_request = 1;
  unsigned char use_enc_pass = 0;
  char pass_file[256];
  char out_file[256] = {};
  char buffer[256] = {};
  int ret, wr_ret;
  ...
  for (i = 1; i < argc && !help; i++) {
	...
	case 'X':
		if (++i >= argc) { help = 1; break; }
        memcpy(prefix_str, argv[i], PREFIX_SIZE);
        memcpy(prefix_str1, argv[i], PREFIX_SIZE);	//used while making final password
		for (prefix_index = 0; prefix_index < PREFIX_SIZE; prefix_index++)
		{
            if ((prefix_str[prefix_index] >= '0') && (prefix_str[prefix_index] <= '9'))
			{
				continue;
			}
			else
			{
				printf("Invalid digit: %c\n", prefix_str[prefix_index]);
				exit(1);
			}
		}
		j = -1;
	...
	case 'R':
		...
		if (argv[i] != NULL)
		{
			rows_request = atoi(&argv[i][0]);
		}
		if (rows_request <= 0 || rows_request > 10)
		{
            printf("Number of Lines: %d must be between 1 to 10\n", rows_request);
			exit(1);
		}
		j = -1;
		break;
	case 'F':
		...
		if (argv[i] != NULL)
		{
            strcpy(out_file, &argv[i][0]);
		}
		else
		{
			printf("Please pass valid argument for -F option: \n");
			exit(1);
		}
		j = -1;
		break;
  ...
  rows = rows_request;  /* Matrix changes */
  if (rows <= 0) {
  ...
  if (regenerate) {
    ...
    memcpy(password1, prefix_str, PREFIX_SIZE);
  } else {
    strcat(prefix_str,"\n");
	memset(password1, 0, sizeof(password1));
	memset(password2, 0, sizeof(password1));
	memcpy(password1, prefix_str, PREFIX_SIZE+1);
	memcpy(password2, prefix_str, PREFIX_SIZE+1);
	/* Matrix changes end */
  }
  if (stdin_is_tty)
    tcsetattr(fileno(stdin), TCSANOW, &term_old);
  ...
  if (use_masterkey) {
    ...
    if (use_enc_pass)
    {
        sprintf(pass_file, "/tmp/.otpw.XXXXXX");
        mkstemp(pass_file);
    }
    else
    {
        strcpy(pass_file, out_file);
    }

    fdesc = fopen(pass_file, "w+");
    if (fdesc == NULL)
	{
  ...
  for (l = 0; l < pages; l++) {
	...
	/* Matrix Changes start */
	ret = sprintf(buffer, "%s%s\n", prefix_str1, password);
	wr_ret = fwrite(buffer, 1, ret, fdesc);
	if (wr_ret < ret)
	{
    ...
    if (use_enc_pass)
    {
        if (mAesEncrypt(pass_file, out_file, (char *)encrypt_pass))
        {
            printf("Error: Password encryption failed\n");
        }
        if (unlink(pass_file))
        {
