...
#include "BTClient.h"    /* Main Application Prototypes and Constants.   */
...
// Global Variable
unsigned int	WriteCharHandle;
unsigned int 	ReadRandomHandle;
unsigned int 	ReadStatusHandle;
unsigned char	EncRandomNo[32];
unsigned char 	DecRandomNo[32];
unsigned char	SecurityKey[32];// = {'a','b','a','b','a','b','a','b','a','b','a','b','a','b','a','b','a','b','a','b','a','b','a','b','a','b','a','b','a','b','a','b'};
unsigned char 	InitVector[32] = {0};

unsigned char 	DeviceName[21];
unsigned char 	Userid[16] = "1541";
unsigned char 	EntryExitInfo = 0;		//Entry
unsigned short 	TmpCrc;
unsigned int 	WriteIndex;
unsigned char	TmpBuf[32];
unsigned char 	EncTmpBuf[32];
unsigned char 	WriteReq[40];
unsigned char 	UserStatus;
BD_ADDR_t		RemoteBDAddr;
volatile unsigned char 	GoAheadF = 0;

unsigned char		ServiceDataBuffer[512];
unsigned int		TotalServiceSizeVal;
...
//               printf("%*s Properties: 0x%02X\r\n", (2*INDENT_LENGTH), "", ParsedGATTData->GATTServiceDiscoveryIndicationData[Index].CharacteristicInformationList[Index1].Characteristic_Properties);
               ...
               // Fill handle data so we can use it.
               if(Index1 == 0)
               {
            	   WriteCharHandle = ParsedGATTData->GATTServiceDiscoveryIndicationData[Index].CharacteristicInformationList[Index1].Characteristic_Handle;
               }
               else if(Index1 == 1)
               {
            	   ReadRandomHandle = ParsedGATTData->GATTServiceDiscoveryIndicationData[Index].CharacteristicInformationList[Index1].Characteristic_Handle;
               }
               else if(Index1 == 2)
               {
            	   ReadStatusHandle = ParsedGATTData->GATTServiceDiscoveryIndicationData[Index].CharacteristicInformationList[Index1].Characteristic_Handle;
               }
...
//      printf("\r\n");
      ...
      switch(EventData->EventType)
      {
         ...
         case detRemoteDeviceFound:
            ...
            if(EventData->EventData.RemoteDeviceFoundEventData.RemoteDeviceProperties.DeviceNameLength > 0)
            {
				if(strncmp(DeviceName, EventData->EventData.RemoteDeviceFoundEventData.RemoteDeviceProperties.DeviceName, EventData->EventData.RemoteDeviceFoundEventData.RemoteDeviceProperties.DeviceNameLength) == 0)
				{
					printf("%s found.\n", DeviceName);
					DEVM_StopDeviceScan();
					RemoteBDAddr = EventData->EventData.RemoteDeviceFoundEventData.RemoteDeviceProperties.BD_ADDR;
					GoAheadF = 1;
				}
            }
         ...
         case detRemoteDevicePropertiesChanged:
           // printf("Remote Device Properties Changed.\r\n");
            ...
            {
            	if(EventData->EventData.RemoteDevicePropertiesChangedEventData.RemoteDeviceProperties.RemoteDeviceFlags & DEVM_REMOTE_DEVICE_FLAGS_DEVICE_LE_SERVICES_KNOWN)
            	{
            		GoAheadF = 1;
            	}
            }
...
//            printf("    Value:            \r\n");
           ...
           // DumpData(ReadAsString, EventData->EventData.ReadResponseEventData.ValueLength, EventData->EventData.ReadResponseEventData.Value);
            if(EventData->EventData.ReadResponseEventData.Handle == ReadRandomHandle)
            {
            	if(EventData->EventData.ReadResponseEventData.ValueLength == 37)
            	{
            		// Copy 32 byte random no.
            		memcpy(EncRandomNo, &EventData->EventData.ReadResponseEventData.Value[5], 32);
            		GoAheadF = 1;
            	}
            }
            if(EventData->EventData.ReadResponseEventData.Handle == ReadStatusHandle)
			{
				if(EventData->EventData.ReadResponseEventData.ValueLength == 6)
				{
					// Copy 1 byte status.
					memcpy(&UserStatus, &EventData->EventData.ReadResponseEventData.Value[5], 1);
					GoAheadF = 1;
				}
			}
            break;
...
int main(int argc, char* argv[])
{
	...
	// Remote Device List related para
	BD_ADDR_t			BD_ADDRList[MAX_REMOTE_DEVICE];
	Class_of_Device_t  	ClassOfDevice;
	unsigned int       	TotalNumberDevices;
	unsigned int       	Filter;
	unsigned int       	Index;
	...
	// Connect to Device related para
	unsigned long 		ConnectFlags = 1;

	// Query service related para
	unsigned long		QueryFlags;
	//   unsigned char		ServiceData[512];
	//   unsigned int			TotalServiceSize;
	DEVM_Parsed_Services_Data_t  ParsedGATTData;

	if(argc != 3)
	{
		printf("Usage: %s [SECURITY_KEY] [DEVICE_NAME_WANT_TO_CONNECT]\n",argv[0]);
		return 1;
	}

	// Fill Security from provided.
	for(idx=0; idx<16; idx++)
	{
		memcpy(&SecurityKey[idx*2], argv[1], 2);
	}

	// Fill Device name from provided
	snprintf(DeviceName, 21, "%s", argv[2]);

	printf("Security Key used in this utility: ");
	DumpData(1, 32, SecurityKey);
	printf("\n");

    //Initializes Bluetooth Platform manager input:-
    //PID, Init info, Unregistration call back and Unregistration paramters
	if(initialized!=1)
	{
	...
	// Query for Bt Device is powered on or not
	Result = DEVM_QueryDevicePowerState();
	if(Result >= 0)
	{
		printf("DEVM_QueryDevicePowerState() Success: %s.\r\n", Result?"On":"Off");
	...
	// If Device is not Powered on then turn on Bt device
	if(poweredon!=1)
	{
	...
	BTPS_MemInitialize(&LocalDeviceProperties, 0, sizeof(LocalDeviceProperties));
	LocalDeviceProperties.DeviceNameLength = (strlen("BTClient"));
	strcpy(LocalDeviceProperties.DeviceName, "BTClient");
	...
	while(1)
	{
		printf("\nEnter Alphanumeric user id to mark punch [Userid] [Entry/Exit= 0/1]\r\n");
		scanf(" %s %d", Userid, &EntryExitInfo);

		if(strcmp("exit", Userid) == 0)
		{
			printf("Exit From Utility");
			break;
		}

		mtick = GetSysTick();
		// Start Device Discovery for 20 second
		if((Result = DEVM_StartDeviceScan(20)) >= 0)
		{
			//printf("DEVM_StartDeviceScan() Success: %d.\r\n", Result);
		}
		else
		{
			// Error attempting to start Device Discovery, inform the user and flag an error.
			printf("DEVM_StartDeviceScan() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
			return 1;
		}
		...
		while(GoAheadF == 0);
		GoAheadF = 0;
		//printf("Going to Connect VegaDevice2\n");
		// During Device Discovery if Bt Device is Found as Provided device name
		// then it store the Bt addr in RemoteBDAddr and stop device discovery.
		// Now connect with available remote Addr.
		if((Result = DEVM_ConnectWithRemoteDevice(RemoteBDAddr, DEVM_CONNECT_WITH_REMOTE_DEVICE_FORCE_LOW_ENERGY)) < 0)
		{
			// Error Connecting With Remote Device, inform the user and flag an error.
			printf("DEVM_ConnectWithRemoteDevice() Failure: [%d] [%s].\r\n", Result, ERR_ConvertErrorCodeToString(Result));
			return 1;
		}
		...
		while(GoAheadF == 0);
		GoAheadF = 0;
		//printf("1. Going to DEVM_QueryRemoteDeviceServices\n");
		// Query for Remote Devices Services.
		if( (Result = DEVM_QueryRemoteDeviceServices(RemoteBDAddr, (DEVM_QUERY_REMOTE_DEVICE_SERVICES_FLAGS_FORCE_UPDATE | DEVM_QUERY_REMOTE_DEVICE_SERVICES_FLAGS_LOW_ENERGY), 0, ServiceDataBuffer, &TotalServiceSizeVal)) < 0)
		{
			printf("1. DEVM_QueryRemoteDeviceServices() Failure: [%d] [%s].\r\n", Result, ERR_ConvertErrorCodeToString(Result));
			return 1;
		}

		while(GoAheadF == 0);
		GoAheadF = 0;
		//printf("2. Going to DEVM_QueryRemoteDeviceServices\n");
		if( (Result = DEVM_QueryRemoteDeviceServices(RemoteBDAddr, DEVM_QUERY_REMOTE_DEVICE_SERVICES_FLAGS_LOW_ENERGY, 512, ServiceDataBuffer, &TotalServiceSizeVal)) >= 0)
		{
			 // Convert the Raw GATT Stream to a Parsed GATT Stream.
			 Result = DEVM_ConvertRawServicesStreamToParsedServicesData((unsigned int)Result, ServiceDataBuffer, &ParsedGATTData);
			 if(!Result)
			 {
				 // Display the Parsed GATT Service Data.
				 DisplayParsedGATTServiceData(&ParsedGATTData);
		...
		//printf("Going to read random number\n");
		// Read Random number from Read random number characteristic.
		if((Result = GATM_ReadValue(GATMCallbackID, RemoteBDAddr, ReadRandomHandle, 0, 1)) >= 0)
		{
			//printf("GATM_ReadValue() Success: Transaction ID = %d.\r\n", Result);
			// Random number copied from GATT Read Response Data into EncRandomNo.
		}
		else
		{
			// Error Connecting With Remote Device, inform the user  and flag an error.
		...
		while(GoAheadF == 0);
		GoAheadF = 0;
		//printf("Going to Prepare Write Request\n");
		//printf("\nEncRandomNo: ");
		//DumpData(0, 32, EncRandomNo);
		//printf("\nSecurityKey: ");
		//DumpData(0, 32, SecurityKey);
		//printf("\nInitVector: ");
		//DumpData(0, 32, InitVector);
		memset(InitVector, 0, 32);
		AesCbc256EncryptData(EncRandomNo, DecRandomNo, 32, SecurityKey, InitVector, AES_DECRYPT);
		//printf("\nDecRandomNo: ");
		//DumpData(0, 32, DecRandomNo);

		// find crc16A of user id.
		TmpCrc = Crc16A(Userid, strlen(Userid), NULL);

		// Prepare tmpBuf for Encryption
		memset(TmpBuf, 0, 32);			// clear tmpBuf.
		memcpy(TmpBuf, &TmpCrc, 2);		// copy first two byte as a CRC16.
		WriteIndex = 2;

		snprintf(&TmpBuf[WriteIndex], 16, "%s", Userid);	// copy user id
		WriteIndex += strlen(&TmpBuf[WriteIndex]);
		//printf("\nTmpBuf ready for Encryption:\n");
		//DumpData(0, 32, TmpBuf);

		// Do Encryption of tmpbuf using received random no;
		AesCbc256EncryptData(TmpBuf, EncTmpBuf, 32, DecRandomNo, InitVector, AES_ENCRYPT);
		//printf("\nEncTmpBuf ready for send:\n");
		//DumpData(0, 32, EncTmpBuf);

		// prepare write request
		WriteReq[0] = 0x01;		// Msg No(2 bytes LSB first).
		WriteReq[1] = 0x00;

		WriteReq[2] = 0x02;		// Number of Field.(1 Byte)
		WriteReq[3] = 0x01;		// First Field length(2 bytes LSB First).
		WriteReq[4] = 0x00;
		WriteReq[5] = 0x20;		// Second Field length(2 bytes LSB First).
		WriteReq[6] = 0x00;
		WriteReq[7] = EntryExitInfo;	// Fill First Field EntryExitinfo

		memcpy(&WriteReq[8], EncTmpBuf, 32);		// Fill 32 byte encrypted user information

		// Write request is prepared send it to Write characteristic in two chunks.
		// First Chunk.
		if((Result = GATM_WriteValueWithoutResponse(GATMCallbackID, RemoteBDAddr, WriteCharHandle, 0, 20, &WriteReq[0])) >= 0)
		{
			//printf("\n1. GATM_WriteValueWithoutResponse() Success: %d.\r\n", Result);
		}
		else
		{
			// Error Connecting With Remote Device, inform the user and flag an error.
			printf("\n1. GATM_WriteValueWithoutResponse() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
		}

		if((Result = GATM_WriteValueWithoutResponse(GATMCallbackID, RemoteBDAddr, WriteCharHandle, 0, 20, &WriteReq[20])) >= 0)
		{
			//printf("\n1. GATM_WriteValueWithoutResponse() Success: %d.\r\n", Result);
		}
		else
		...
		do
		{
			// now send read status request
			if((Result = GATM_ReadValue(GATMCallbackID, RemoteBDAddr, ReadStatusHandle, 0, 1)) >= 0)
			{
				// printf("GATM_ReadValue() Success: Transaction ID = %d.\r\n", Result);
				// user request Status copied from GATT Read Response Data into UserStatus.
			}
			else
			{
				// Error Connecting With Remote Device, inform the user and flag an error.
			...
			// now as per UserStatus Display status.
			switch(UserStatus)
			{
				case RESP_STATUS_NONE:
					printf("User Status: RESP_STATUS_NONE\n");
					break;

				case RESP_USER_ALLOWED:
					printf("User Status: RESP_USER_ALLOWED\n");
					break;

				case RESP_WAITING_SECOND_CRED:
					printf("User Status: RESP_WAITING_SECOND_CRED\n");
					break;

				case RESP_DEVICE_BUSY:
					printf("User Status: RESP_DEVICE_BUSY\n");
					break;

				case RESP_WRONG_DATA:
					printf("User Status: RESP_WRONG_DATA\n");
					break;

				case RESP_INSUFFICIENT_DATA:
					printf("User Status: RESP_INSUFFICIENT_DATA\n");
					break;

				case RESP_WRONG_FORMAT:
					printf("User Status: RESP_WRONG_FORMAT\n");
					break;

				case RESP_USER_DENIED:
					printf("User Status: RESP_USER_DENIED\n");
					break;

				default:
					printf("Default case: User Status: Undefined\n");
					break;
			}

			usleep(500000);
		}while(UserStatus == RESP_DEVICE_BUSY);

		// DisConnect Remote Device
		DEVM_DisconnectRemoteDevice(RemoteBDAddr, DEVM_DISCONNECT_FROM_REMOTE_DEVICE_FLAGS_LOW_ENERGY);

		printf("Time Required to mark punch is time[%u ms]\n", ElapsedTick(mtick));
	}
...
//***************************************************************************
//	AesCbc256EncryptData()
//	Param:
//		IN :	NONE
//		OUT:	NONE
//	Returns:	Success / fail
//	Description:
//			This function will perform Encryption/Decryption using
...
void AesCbc256EncryptData(unsigned char* inData, unsigned char* outData, unsigned int dataLen, unsigned char* keyPtr, unsigned char* iv, unsigned char mode)
{
	...
	if(mode == AES_ENCRYPT)
	{
		AES_set_encrypt_key(keyPtr, AES_256_KEY_SIZE_IN_BITS, &aesKey);
		AES_cbc_encrypt(inData, outData, dataLen, &aesKey, iv, AES_ENCRYPT);
	}
	else
	{
		AES_set_decrypt_key(keyPtr, AES_256_KEY_SIZE_IN_BITS, &aesKey);
		AES_cbc_encrypt(inData, outData, dataLen, &aesKey, iv, AES_DECRYPT);
	}
}
...
//	Returns:	Success / fail
//	Description:
//			This function will find CRC16
//**************************************************************************
unsigned short Crc16A(unsigned char* dataPtr, unsigned int len, unsigned char * crcPtr)
{
	unsigned int 	crcInit,tempIndex1, tempIndex2;
	unsigned short	tempCrc;

	// Initialise crcInit.
	crcInit = 0xc6c6;
	for (tempIndex1=0; tempIndex1<len; tempIndex1++)
	{
		crcInit = (Reflect(dataPtr[tempIndex1],8)<<8) ^ crcInit;
		for (tempIndex2=0; tempIndex2<8; tempIndex2++)
		{
			if (crcInit & 0x8000)
			{
	...
	tempCrc = Reflect((unsigned short)crcInit,16);

	// Fill Resulting CRC in provided Array.
	if(crcPtr)
	{
		crcPtr[0] = (unsigned char) tempCrc;			// LSByte
		crcPtr[1] = (unsigned char) (tempCrc >> 8);		// MSByte
	}
	return(tempCrc);
}
...
//	Returns:	Success / fail
//	Description:
//			This function will reflect data for CRC16
//**************************************************************************
unsigned short Reflect(unsigned short number,unsigned int size)
{
	// reflect by bit
	unsigned short 	reflectedno = 0;
	unsigned int 	tempInx;
	for(tempInx=0; tempInx<size; tempInx++)
	{
	  reflectedno = (reflectedno*2)+(number%2);
	  number = number/2;
	}
	return reflectedno;
}
...
UINT32 GetSysTick(void)
{
	...
	{
		currMsecCnt = (timeVal.tv_sec * VAL_OF_1MS) + (timeVal.tv_nsec / NANO_TO_MILI_SEC);
...
UINT32 ElapsedTick(UINT32 markTick)
{
	...
	GetSysTick();
	return(GetAbsTickDiff(sysTick, markTick));
}
